...
## Model policy
Current brain: `gpt-5.5` via `openai-codex` for officer synthesis and verification drills. If `gpt-5.5` fails, fall back to `gpt-5.4-mini` on `openai-codex` and report. Gemini/frontier restored when funded. Parent delegate tasks may use cheaper models only for draft/internal work verified by S1/EDEN before reporting.
